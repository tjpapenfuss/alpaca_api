...
class InvestmentForecastingModel:
    ...
    def run_simulation(self):
        ...
        for i, investment_date in enumerate(self.investment_dates):
            ...
            self._check_and_rebalance(portfolio, prices, closest_date, transactions, sold_tickers)
    ...
    def _check_and_rebalance(self, portfolio, prices, date, transactions, sold_tickers=None):
        """
        ...
        if not portfolio['holdings'] or not hasattr(self, 'portfolio_history') or len(self.portfolio_history) < 3:
            return
        ...
        if self.last_rebalance_date is None:
            ...
            first_date = pd.to_datetime(self.start_date)
            if (current_date - first_date).days >= 90:  # At least 90 days after start
                should_rebalance_time = True
        else:
            last_rebalance = pd.to_datetime(self.last_rebalance_date)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'start_date': '2021-01-01',          # Start date for simulation
        'end_date': '2025-03-03',            # End date for simulation
        ...
        'rebalance_frequency': 'quarterly',  # Add this parameter
        'rebalance_threshold': 5  # Add this parameter - rebalance if any position drifts by 5%
    }
